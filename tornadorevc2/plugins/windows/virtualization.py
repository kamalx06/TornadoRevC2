...
function CloudProbe($url,$hdr){
  try{$r=Invoke-WebRequest -Uri $url -Headers $hdr -TimeoutSec 1 -UseBasicParsing; return $r.Content.Substring(0,[Math]::Min(120,$r.Content.Length))}catch{return ''}
}
...
foreach($pair in @(@('vmci','vmware'),@('vboxguest','virtualbox'),@('vboxsf','virtualbox'),@('vmrawdsk','vmware'),@('xenevtchn','xen'),@('viogpusc','qemu'),@('virtio','qemu'),@('HvHost','hyperv'))){
  if(Test-Path ("HKLM:\SYSTEM\CurrentControlSet\Services\{0}" -f $pair[0])){Hit $det $pair[1] ('service key: '+$pair[0]) 10}
}
try{if(Get-ItemProperty 'HKLM:\SOFTWARE\Microsoft\Virtual Machine\Guest\Parameters' -EA 0){Hit $det 'hyperv' 'Hyper-V guest parameters registry' 20}}catch{}
...
$runtime=''; if(DetOn $det 'docker'){$runtime='Docker Desktop'} elseif(DetOn $det 'windows_containers'){$runtime='Windows Containers'}
$orchestrator = $(if(DetOn $det 'kubernetes'){'Kubernetes'}else{''})
$virt=''; foreach($p in @(@('vmware','VMware'),@('virtualbox','VirtualBox'),@('hyperv','Hyper-V'),@('qemu','QEMU'),@('xen','Xen'))){if(DetOn $det $p[0]){$virt=$p[1];break}}
$cloudp=''; foreach($k in @('cloud_aws','cloud_azure','cloud_gcp')){if(DetOn $det $k){$cloudp=$k.Replace('cloud_','').ToUpper();break}}
...
elseif($virt -or $cloudp){$envType='Virtual Machine'} else{$envType='Physical Host'}
...
$hostAccess='None detected'; if(Test-Path '\\.\pipe\docker_engine'){$hostAccess='Docker engine pipe present'}
...
$nested='Not detected'
try{if((Get-ItemProperty 'HKLM:\SOFTWARE\Microsoft\Virtual Machine\Guest\Parameters' -EA 0) -and ($virt -or $cloudp)){$nested='Hyper-V guest parameters present inside VM'}}catch{}
...
$result=[ordered]@{
  environment_type=$envType; runtime=$runtime; orchestrator=$orchestrator
  namespace=$env:POD_NAMESPACE; node=$env:NODE_NAME
  container_id=$(if($inContainer){$env:COMPUTERNAME}else{''})
  host_relationship=$hostRel; virtualization_type=$virt; cloud_provider=$cloudp
  host_access=$hostAccess; container_privileges=$(if($inContainer){'Container context'}else{'N/A'})
  nested_virtualization=$nested; hardware_virtualization=$(if($virt){'Guest VM environment'}else{'Not detected'})
  sandbox_indicators=($(if($sandbox){$sandbox -join '; '}else{'None'}))
  confidence=$confidence; indicators=$indicators; namespaces=@{}; sockets=$sockets
  artifacts=$artifacts; cloud_metadata=$cloud; detections=$det
}
