...
class Updater:
    ...
    def handle_command(self, cmd_parts):
        ...
        if cmd_parts[0].lower() != 'update':
            return False
        if len(cmd_parts) > 1 and cmd_parts[1].lower() == 'unstable':
            self.run_unstable_update()
        else:
            self.run_update()
        return True
    ...
    def run_unstable_update(self):
        """Run update from the development branch."""
        if self._busy:
            self._print_warning('Update already in progress.')
            return
        ...
        try:
            with UpdateLock():
                ...
                # For unstable, verify development branch instead of main
                branch_ok, current_branch = verify_trusted_branch(repo_root, self._run_git, development=True)
                if not branch_ok:
                    self._print_error(
                        WRONG_BRANCH_MSG.format(branch=DEVELOPMENT_BRANCH, current=current_branch)
                    )
                    self.audit.log(
                        'UPDATE_ABORTED',
                        reason='wrong_branch',
                        trusted_branch=DEVELOPMENT_BRANCH,
                        current_branch=current_branch,
                    )
                    return
                ...
                self._print_info(
                    f'Fetching updates from {OFFICIAL_REPO_URL} ({DEVELOPMENT_BRANCH})...'
                )
                fetch = self._run_git(
                    'fetch', '--quiet', 'origin', DEVELOPMENT_BRANCH,
                    cwd=repo_root,
                    timeout=GIT_TIMEOUT_FETCH,
                )
                if fetch.returncode != 0:
                ...
                target_revision = get_trusted_branch_tip(repo_root, self._run_git, development=True)
                if target_revision is None:
                    self._print_error(
                        f'Unable to determine the trusted branch tip for origin/{DEVELOPMENT_BRANCH}.'
                    )
                    self.audit.log('UPDATE_ABORTED', reason='target_identification_failed')
                    return
                ...
                print(f"Current commit: {_short_revision(previous_revision)}")
                print(f"Target commit:  {_short_revision(target_revision)}")
                print(f"Source branch:  {DEVELOPMENT_BRANCH}")
                print(f"Commits:        {commit_count}")
                if shortstat:
                    print(f"Changes:        {shortstat}")
                if commits:
